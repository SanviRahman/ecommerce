...
@section('content')
    <div id="page-manager"
         ...
         data-mode="active"
         data-index-url="{{ route('admin.reviews.index') }}"
         data-create-url="{{ route('admin.reviews.create') }}"
         data-bulk-url="{{ route('admin.reviews.multiple_action') }}"
         data-sort-url="{{ route('admin.reviews.sort') }}"
         data-trash-url="{{ route('admin.reviews.trashed') }}">
        ...
        <div class="d-flex align-items-center flex-wrap mb-3" style="gap: 6px;">
            ...
            @can('review_trash')
                <a href="{{ route('admin.reviews.trashed') }}" class="btn btn-outline-danger btn-sm font-weight-bold shadow-sm">
            ...
            <div class="card-body px-3 py-2">
                <div class="row align-items-end">
                    <div class="col-md-10 mb-2 mb-md-0">
                        <label for="table_search" class="text-muted small font-weight-bold text-uppercase mb-1">Search Reviews</label>
                        <div class="input-group input-group-sm">
                            <input type="text" id="table_search" class="form-control shadow-none" autocomplete="off" placeholder="Search by reviewer name, title or review text...">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white px-3 py-3 border-bottom">
                    <h3 class="card-title font-weight-bold text-dark mb-0">
                        <i class="fas fa-star text-primary mr-1"></i>Reviews List (Drag & Drop to Sort)
                    </h3>
                </div>
                <div class="card-body p-0" id="content-wrapper" style="min-height: 340px;">
                    @include('admin.reviews.partials.table', ['reviews' => $reviews, 'isTrash' => false])
                </div>
            </div>
        @else
            <div class="alert alert-warning border-0 shadow-sm mt-4 font-weight-bold">
                <i class="fas fa-exclamation-triangle mr-2"></i> You do not have permission to view reviews.
            </div>
        @endcan
    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title font-weight-bold text-primary" id="modal-title">Review Management</h5>
...
@section('js')
    @include('admin.reviews.partials.script')
@endsection