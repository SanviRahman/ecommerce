...
@section('content')
    <div id="page-manager"
         ...
         data-mode="trash"
         data-index-url="{{ route('admin.meta-pixel-scripts.trashed') }}"
        ...
        <div class="d-flex align-items-center flex-wrap mb-3" style="gap: 6px;">
            @can('meta_pixel_script_list')
                <a href="{{ route('admin.meta-pixel-scripts.index') }}" class="btn btn-secondary btn-sm font-weight-bold shadow-sm">
                    <i class="fas fa-arrow-left mr-1"></i> Back to Scripts
                </a>
            @endcan

            <select id="bulk_action" class="form-control form-control-sm shadow-none" style="width: 210px;">
                <option value="">-- Bulk Actions --</option>
                @can('meta_pixel_script_restore')
                    <option value="restore">Restore Selected</option>
                @endcan
                @can('meta_pixel_script_force_delete')
            ...
            <div class="card-body px-3 py-2">
                <div class="row align-items-end">
                    <div class="col-md-10 mb-2 mb-md-0">
                        <label for="table_search" class="text-muted small font-weight-bold text-uppercase mb-1">Search Trashed Scripts</label>
                        <div class="input-group input-group-sm">
                            <input type="text" id="table_search" class="form-control shadow-none" autocomplete="off" placeholder="Search script name...">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white px-3 py-3 border-bottom text-danger">
                    <h3 class="card-title font-weight-bold mb-0">
                        <i class="fas fa-trash-alt mr-2"></i>Trash Bin (Meta Pixel Scripts)
                    </h3>
                </div>

                <div class="card-body p-0" id="content-wrapper" style="min-height: 340px;">
                    @include('admin.meta-pixel-scripts.partials.table', [
                        'scripts' => $scripts,
                        'isTrash' => true,
                    ])
                </div>
        ...
        @else
            <div class="alert alert-warning border-0 shadow-sm mt-4 font-weight-bold">
                <i class="fas fa-exclamation-triangle mr-2"></i> You do not have permission to view trashed scripts.
            </div>
        @endcan
    </div>
@endsection
