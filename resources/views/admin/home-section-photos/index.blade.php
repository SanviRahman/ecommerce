...
@section('content')
    <div id="page-manager"
         ...
         data-mode="active"
         data-index-url="{{ route('admin.home-section-photos.index') }}"
         data-create-url="{{ route('admin.home-section-photos.create') }}"
         data-bulk-url="{{ route('admin.home-section-photos.multiple_action') }}"
         data-sort-url="{{ route('admin.home-section-photos.sort') }}"
         data-trash-url="{{ route('admin.home-section-photos.trashed') }}">

        <!-- Top Action Bar -->
        <div class="d-flex align-items-center flex-wrap mb-3" style="gap: 6px;">
            @can('home_section_photo_create')
            ...
            @can('home_section_photo_trash')
                <a href="{{ route('admin.home-section-photos.trashed') }}" class="btn btn-outline-danger btn-sm font-weight-bold shadow-sm">
            ...
            <div class="card-body px-3 py-2">
                <div class="row align-items-end">
                    <div class="col-md-3 mb-2 mb-md-0">
                        <label for="filter_section" class="text-muted small font-weight-bold text-uppercase mb-1">Filter By Section</label>
                        <select id="filter_section" class="form-control form-control-sm shadow-none">
                            <option value="">All Sections</option>
                            <option value="our_products">Our Products</option>
                            <option value="after_what_we_offer">After What We Offer</option>
                        </select>
                    </div>

                    <div class="col-md-7 mb-2 mb-md-0">
                        <label for="table_search" class="text-muted small font-weight-bold text-uppercase mb-1">Search Photos</label>
                        <div class="input-group input-group-sm">
                            <input type="text" id="table_search" class="form-control shadow-none" autocomplete="off" placeholder="Search by title or caption...">
        ...
        <!-- Main Table Section -->
        @can('home_section_photo_list')
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white px-3 py-3 border-bottom">
                    <h3 class="card-title font-weight-bold text-dark mb-0">
                        <i class="fas fa-images text-primary mr-1"></i>Home Section Photos List (Drag & Drop to Sort)
                    </h3>
                </div>
                <div class="card-body p-0" id="content-wrapper" style="min-height: 340px;">
                    @include('admin.home-section-photos.partials.table', ['photos' => $photos, 'isTrash' => false])
                </div>
            </div>
        @else
            <div class="alert alert-warning border-0 shadow-sm mt-4 font-weight-bold">
                <i class="fas fa-exclamation-triangle mr-2"></i> You do not have permission to view home section photos.
            </div>
        @endcan
    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title font-weight-bold text-primary" id="modal-title">Home Section Photo Management</h5>
...
@section('js')
    @include('admin.home-section-photos.partials.script')
@endsection