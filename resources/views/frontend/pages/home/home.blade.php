...
<section id="our-products" class="home-products section-space">
    <div class="container">
        ...
        {{-- Desktop / tablet: existing 3-products-per-slide carousel --}}
        <div
            id="productShowcaseCarousel"
            class="carousel slide reveal product-carousel-desktop d-none d-md-block"
            data-ride="carousel"
            data-interval="5000"
            data-touch="true"
        >
        ...
        {{-- Mobile: single product, automatic right-to-left carousel --}}
        <div
            id="productShowcaseMobileCarousel"
            class="carousel slide reveal product-carousel-mobile d-md-none"
            data-ride="carousel"
            data-interval="3200"
            data-pause="false"
            data-wrap="true"
        ...
        >
            <div class="carousel-inner">
                @forelse($products as $product)
                    @php
                        $mobileProductImage = $product->getFirstMediaUrl('featured_image')
                            ?: asset('frontend/images/home/product-spc.webp');

                        $mobileProductUrl = '#';

                        if (\Illuminate\Support\Facades\Route::has('products.show')) {
                            $mobileProductUrl = route('products.show', ['product' => $product->slug]);
                        } elseif (\Illuminate\Support\Facades\Route::has('products.index')) {
                            $mobileProductUrl = route('products.index');
                        }
                    @endphp

                    <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                        <a href="{{ $mobileProductUrl }}" class="product-showcase-card mobile-product-card">
                            <div class="product-media">
                                <img
...
{{-- Reusable Signature / TMC section --}}
...
@include('frontend.pages.home.home_css')

@include('frontend.pages.home.home_script')
